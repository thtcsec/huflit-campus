...
public record CancelEventCommand(Guid EventId, string? Reason = null) : IRequest<Result<EventDetailDto>>;

public class CancelEventCommandHandler : IRequestHandler<CancelEventCommand, Result<EventDetailDto>>
{
    ...
    public async Task<Result<EventDetailDto>> Handle(CancelEventCommand request, CancellationToken cancellationToken)
    {
        ...
        var result = entity.Cancel(request.Reason);
        if (result.IsFailure)
        ...
        var notifications = registrations.Items
            ...
            .Select(r => Notification.Create(
                ...
                "Event cancelled",
                $"The event \"{entity.Title}\" has been cancelled."))
            .ToList();
